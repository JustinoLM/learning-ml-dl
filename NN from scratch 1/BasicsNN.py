...
# Funciones de activacioncapa1.adelante(X)
#capa2.adelante(capa1.salida)



'''
Una función de activación en redes neuronales es una fórmula matemática que decide si una neurona debe activarse o no, 
es decir, si la señal de entrada debe pasar a la siguiente capa de la red. 
Su principal función es introducir no linealidad en la salida de una neurona 
para que la red neuronal pueda aprender y modelar datos complejos, como imágenes, texto o señales de audio.
'''

'''
ReLU (Rectified Linear Unit): Devuelve 0 si la entrada es negativa, y la misma entrada si es positiva.
ReLU es computacionalmente más eficiente porque solo requiere una operación matemática simple.

ReLU ayuda a que los gradientes sean más grandes y consistentes, lo que puede acelerar el proceso de aprendizaje.
Si la entrada es negativa, el gradiente será cero, y no habrá actualización en los pesos conectados a esa neurona. 
Esto puede llevar a algunas neuronas a "morir" durante el entrenamiento si nunca se activan 
(siempre reciben entradas negativas).

'''

class ReLU_Activacion:
	def adelante(self, entradas):
		self.salidas = np.maximum(0, entradas)

import Creador_dataset as CD

X,y = CD.crear_dataset(100,3)


# La funcion crear dataset solo tiene puntos en X y Y osea 2 caracteristicas

## Declaramos las clases
capa1 = Capa_Dense(2,5)
activacion1 = ReLU_Activacion()

capa1.adelante(X)
print("\n# Salida antes de la activacion")
print(capa1.salidas)

# Vamos a notar que no hay ninguna salida negativa

activacion1.adelante(capa1.salidas)
print("\n# Salida despues de la activacion")
print(activacion1.salidas)
